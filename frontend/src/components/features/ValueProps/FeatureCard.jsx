...
export function FeatureCard({ icon, title, text }) {
  return (
    <div
      className="group relative bg-black/10 p-8 rounded-3xl text-center flex flex-col items-center h-full
                 transition-all duration-300 ease-in-out
                 border border-white/10 shadow-lg
                 hover:!bg-black/20 hover:!-translate-y-2 hover:!scale-105 hover:!border-white/20 hover:!shadow-xl"
      style={{
        willChange: 'transform, background-color, box-shadow, border-color'
      }}
    >
      ...
      <h3 className="font-brand text-2xl font-bold text-white uppercase mb-4 transition-all duration-300
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.filter = 'drop-shadow(0 0 10px rgba(212,175,55,0.5))';
          }}
          onMouseLeave={(e) => {
