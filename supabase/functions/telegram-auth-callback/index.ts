...
// ---------- ENV (новые имена с фолбэком на старые) ----------
const SB_URL            = Deno.env.get('SB_URL')              ?? Deno.env.get('SUPABASE_URL')!;
const SB_ANON_KEY       = Deno.env.get('SB_ANON_KEY')         ?? Deno.env.get('SUPABASE_ANON_KEY')!;
const SB_SERVICE_ROLE   = Deno.env.get('SB_SERVICE_ROLE_KEY') ?? Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;
const TG_CLIENT_SECRET  = Deno.env.get('TELEGRAM_CLIENT_SECRET');
const SHADOW_SECRET     = Deno.env.get('SHADOW_PASSWORD_SECRET');
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!tg) {
      return json(req, { success: false, error: 'Отсутствуют данные авторизации Telegram.' }, { status: 400 });
    }

    const telegramId = Number(tg.id);
    if (!telegramId) {
      return json(req, { success: false, error: 'Невалидный Telegram ID.' }, { status: 400 });
    }

    // — prod-безопасность —
    if (!DEBUG) {
      if (!TG_CLIENT_SECRET) {
        return json(req, { success: false, error: 'Нет TELEGRAM_CLIENT_SECRET.' }, { status: 500 });
      }
      if (!isFreshAuth(tg.auth_date)) {
        return json(req, { success: false, error: 'Просроченные данные авторизации.' }, { status: 401 });
      }
      const ok = await verifyTelegramHash(tg, TG_CLIENT_SECRET);
      if (!ok) {
        return json(req, { success: false, error: 'Ошибка проверки подписи Telegram.' }, { status: 401 });
      }
    }

    if (!SHADOW_SECRET) {
      return json(req, { success: false, error: 'Нет SHADOW_PASSWORD_SECRET.' }, { status: 500 });
    }
    ...
    if (!res.ok) {
      ...
      return json(req, { success: false, error: `Password grant failed: ${res.status} ${text}` }, { status: 500 });
    }
